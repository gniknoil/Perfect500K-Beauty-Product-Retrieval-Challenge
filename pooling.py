...
__all__=['AMAC_Pooling','Rmac_Pooling','Mac_Pooling','SPoC_pooling','Ramac_Pooling','Grmac_Pooling','Soft_Grmac']
...
def soft_ramac(x, L=3, eps=1e-6, p=1, sigma_factor=2.0):
    ovr = 0.4 # desired overlap of neighboring regions
    steps = torch.Tensor([2, 3, 4, 5, 6, 7]) # possible regions for the long dimension
    ...
    x_min=x.sum(1).min()
    # Gaussian prior
    sigma = min(H, W) / 2.0 / sigma_factor
    m = torch.Tensor(range(W))
    n = torch.Tensor(range(H))
    spatial_H = torch.exp(-((n - H / 2.0) ** 2) / 2.0 / (sigma ** 2))
    spatial_H = spatial_H.unsqueeze(0).transpose(1,0).cuda()
    spatial_W = torch.exp(-((m - W / 2.0) ** 2) / 2.0 / (sigma ** 2))
    spatial_W = spatial_W.unsqueeze(0).cuda()
    spatial_weight = spatial_H * spatial_W * (x.sum(1)-x_min)
    threshold=spatial_weight.pow(p).mean().pow(1/p)+x_min
...
class Soft_Grmac(nn.Module):
    def __init__(self, L=3, eps=1e-6, p=1, sigma_factor=3.0):
        super(Soft_Grmac,self).__init__()
        self.L=L
        self.eps=eps
        self.p=p
    ...
    def forward(self, x):
        out=soft_ramac(x, L=self.L, eps=self.eps, p=self.p, sigma_factor=self.sigma_factor)
        return out.squeeze(-1).squeeze(-1)

if __name__=='__main__':
    a=torch.randn(3,64,10,10)
    b=torch.randn(3,64,15,15)

    soft_Grmac = Soft_Grmac()
    feat = soft_Grmac(a.cuda())
    print(feat.size())
    AMAC=AMAC_Pooling()
    feature1_max=AMAC(a)
    print(feature1_max.size())